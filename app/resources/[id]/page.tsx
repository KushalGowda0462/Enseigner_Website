...
import { FullBlogs, recentPosts as relatedPosts} from "@/data/data"

export default async function BlogPost({params}:{
  params: Promise<{ id: string }>
})  {
  const {id}=await params
  const currBlog = FullBlogs[Number(id)] || FullBlogs[0]
  return (
    <article className="min-h-screen bg-background">
      ...
      <div className="site-container py-12">
        ...
        <div className="span-centered-10 space-y-8">
          {currBlog.content.slice(0, 3).map((content, idx) => (<p key={idx} className="text-lg leading-relaxed text-foreground">
            {content.para}
          </p>))}
      ...
      <section className="site-container py-12">
        <h2 className="text-2xl font-bold mb-8 text-foreground">Other Interesting Blogs</h2>
        <div className="grid-12">
