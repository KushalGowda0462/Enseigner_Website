...
import { FullCaseStudies, recentPosts as relatedPosts} from "@/data/data"

export default async function BlogPost({params}:{
  params: Promise<{ id: string }>
})  {
  const {id}=await params
  const currBlog = FullCaseStudies[Number(id)] || FullCaseStudies[0]
  return (
    <article className="min-h-screen bg-background">
      ...
      <section className="site-container py-12">
        <h2 className="text-2xl font-bold mb-8 text-foreground">Other Interesting Case Studies</h2>
        <div className="grid-12">
